...
@push('seo')
    <meta name="description"
          content="{{ $product->about ?? \Illuminate\Support\Str::limit(strip_tags($product->description), 160) }}">
    ...
    <meta property="og:description"
          content="{{ $product->about ?? \Illuminate\Support\Str::limit(strip_tags($product->description), 200) }}">
            ...
            <div class="product-detail-grid">
                <div class="product-image-gallery" data-aos="fade-right">
                    <div class="main-image">
                        <img
                            src="{{ $product->displayImage() ?? 'https://placehold.co/600x600/007bff/white?text=' . urlencode($product->name) }}"
                            alt="{{ $product->name }}">
                    </div>
                </div>
                ...
                <div class="product-info" data-aos="fade-left">
                        ...
                        @if(config('filament-ecommerce.enable_pricing'))
                            @if($product->discount > 0)
                                <div class="price-wrapper">
                                    ...
                                    <span
                                        class="price discounted">₦{{ number_format($product->price - ($product->price * ($product->discount / 100)), 2) }}</span>
                    ...
                    @if($product->is_in_stock)
                        <div class="add-to-cart-section"
                            ...
                            @livewire('add-to-cart-button', ['productId' => $product->id, 'large' => true],
                            key('add-to-cart-large-'.$product->id))
                        </div>
                    @endif
